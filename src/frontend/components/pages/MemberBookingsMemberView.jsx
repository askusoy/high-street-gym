...
export const MemberBookingsMemberView = () => {
  ...
  useEffect(() => {
    console.log("Loading booking list")
      fetch(`/api/member_activity_bookings/user_view/${params.user_id}`) //linking thing need to be member user id as well?? e.g., Link to ... `$member_user_id`? and fetch session?
        .then((res) => res.json())
        .then((res) => {
